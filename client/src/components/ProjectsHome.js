...
const ProjectsHome = ({ user }) => { // Accept user prop
  ...
  const [newProjectThemeAngle, setNewProjectThemeAngle] = useState('');
  const [newProjectPartnership, setNewProjectPartnership] = useState('');
  ...
  const addProject = async (e) => {
    ...
    try {
      ...
      const body = {
        ...
        themeAngle: newProjectThemeAngle,
        partnership: newProjectPartnership,
        questions: newProjectQuestions,
      };
      await axios.post(`${process.env.REACT_APP_API_URL}/api/projects`, body, config);
